...
class ReceiveSocket:
	...
	def recv_data(self, size, overflow=b""):
		...
		data = overflow # Any left over data from last iteration
		start_time = time.perf_counter()

		while len(data) < size: # Receive data until we've got everything
			data += self.conn.recv(4096)
			if time.perf_counter() - start_time > 0.5: # Timeout after 500ms
				raise SocketTimeout
